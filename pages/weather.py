import streamlit as st
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
import os

# --------------------------------------------------------------------------------
# 1. 설정 및 한글 폰트 이슈 대응 (그래프 깨짐 방지 위해 영문 라벨 사용 권장)
# --------------------------------------------------------------------------------
st.set_page_config(page_title="기온 추세 분석", layout="wide")

st.title("🌡️ 기온 데이터 시각화 및 추세선 분석")
st.markdown("데이터 파일(`temperature_data.csv`)이 같은 폴더에 있다면 자동으로 불러옵니다.")

# --------------------------------------------------------------------------------
# 2. 데이터 불러오기 (파일이 없으면 예시 데이터 생성)
# --------------------------------------------------------------------------------
file_name = 'temperature_data.csv'

@st.cache_data
def load_data():
    if os.path.exists(file_name):
        # 파일이 있으면 읽기 (인코딩은 상황에 따라 'cp949'나 'utf-8' 사용)
        try:
            df = pd.read_csv(file_name, encoding='utf-8')
        except UnicodeDecodeError:
            df = pd.read_csv(file_name, encoding='cp949')
        return df
    else:
        # 파일이 없으면 예시 데이터 생성 (사용자 테스트용)
        st.warning(f"⚠️ '{file_name}' 파일을 찾을 수 없어 예시 데이터를 생성하여 보여줍니다.")
        data = {
            'Year': range(2014, 2024),
            'Min': [-12.5, -13.0, -11.5, -12.8, -10.5, -9.8, -10.2, -8.5, -9.0, -7.5],
            'Max': [34.5, 35.2, 36.0, 35.8, 37.5, 36.8, 37.2, 38.0, 38.5, 39.1],
            'Avg': [12.5, 12.7, 12.9, 13.1, 13.4, 13.6, 13.8, 14.1, 14.3, 14.6]
        }
        return pd.DataFrame(data)

df = load_data()

# 사용자 파일의 컬럼명이 다를 수 있으므로 매핑 (필요시 수정하세요)
# 예: 사용자의 csv 파일 컬럼이 '연도', '평균' 등으로 되어있다면 아래를 수정
col_year = 'Year'   # 연도 컬럼명
col_min = 'Min'     # 절대 최저 컬럼명
col_max = 'Max'     # 절대 최고 컬럼명
col_avg = 'Avg'     # 평균 기온 컬럼명

# 데이터 확인
with st.expander("📄 데이터 원본 보기"):
    st.dataframe(df)

# --------------------------------------------------------------------------------
# 3. 추세선 계산 (Numpy Polyfit 사용 - 별도 라이브러리 설치 불필요)
# --------------------------------------------------------------------------------
# x축: 연도, y축: 평균 기온
x = df[col_year]
y = df[col_avg]

# 1차 방정식 (y = ax + b)의 기울기(a)와 절편(b) 계산
# 기울기(slope)가 곧 '전년 대비 평균 기온 상승값'의 추세입니다.
slope, intercept = np.polyfit(x, y, 1)

# 추세선 함수 생성
trend_func = np.poly1d((slope, intercept))

# 상승값 계산 결과 표시
st.subheader("📊 분석 결과")
st.write(f"**평균 기온 데이터를 기반으로 계산한 연평균 상승 추세:** 약 `{slope:.4f} ℃/년`")
if slope > 0:
    st.write("👉 전년 대비 기온이 **상승**하는 추세입니다.")
else:
    st.write("👉 전년 대비 기온이 **하강**하는 추세입니다.")

# --------------------------------------------------------------------------------
# 4. 그래프 그리기
# --------------------------------------------------------------------------------
fig, ax = plt.subplots(figsize=(12, 6))

# A. 절대 최저값 (파란색 점선)
ax.plot(df[col_year], df[col_min], label='Absolute Min Temp', color='blue', linestyle='--', alpha=0.5)

# B. 절대 최고값 (초록색 점선)
ax.plot(df[col_year], df[col_max], label='Absolute Max Temp', color='green', linestyle='--', alpha=0.5)

# C. 평균 기온값 (검은색 실선)
ax.plot(df[col_year], df[col_avg], label='Average Temp', color='black', linewidth=1.5, marker='o')

# D. 추세선 (빨간색 실선) - 요청하신 부분
# 평균 기온 상승값(기울기)에 맞춰 그려짐
ax.plot(df[col_year], trend_func(x), label=f'Trend Line (Rise: {slope:.2f}/yr)', color='red', linewidth=3)

# 그래프 꾸미기
ax.set_title('Temperature Trend Analysis', fontsize=15)
ax.set_xlabel('Year')
ax.set_ylabel('Temperature (℃)')
ax.legend()
ax.grid(True, linestyle=':', alpha=0.6)

# Streamlit에 그래프 출력
st.pyplot(fig)
